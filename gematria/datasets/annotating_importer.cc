...
absl::StatusOr<uint64_t> AnnotatingImporter::GetELFOffset(
    const llvm::object::ELF64LEObjectFile *elf_object) {
  uint64_t file_offset = 0;
  bool found_file_offset = false;
...
AnnotatingImporter::GetBlocksFromELF() {
  ...
  const absl::StatusOr<uint64_t> file_offset = GetELFOffset(*elf_object);
  if (!file_offset.ok()) {
    return file_offset.status();
  }
    ...
    for (const llvm::object::BBAddrMap::BBRangeEntry &bb_range :
      ...
      for (const llvm::object::BBAddrMap::BBEntry &bb : bb_range.BBEntries) {
        ...
        const auto &basic_block =
            GetELFSlice(*elf_object, begin_idx, end_idx, *file_offset);
        if (!basic_block.ok()) {
  ...
  for (const auto &event : perf_data_proto.events()) {
    ...
    for (int branch_idx = 0; branch_idx + 1 < branch_stack.size();
      ...
      if (index_map.count(block_range)) {
        ...
        const auto block =
            GetELFSlice(*elf_object, block_begin, block_end, *file_offset);
        if (!block.ok()) {
          return block.status();
        }
